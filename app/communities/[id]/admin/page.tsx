...
export default function CommunityAdminPage({
  ...
  const loadCommunityData = async (id: string) => {
    try {
      ...
      if (communityData) {
          ...
          try {
            ...
            if (
              ...
              locationData = {
                ...
                address:
                  parsed.address || parsed.Address || parsed.full_address || "",
              };
            }
          } catch (e) {
        ...
        // First try to get from categories relationship
        if (
          communityData.categories &&
          (communityData.categories as any).name
        ) {
          categoryValue = (communityData.categories as any).name;
          tagsValue = [categoryValue];
        } else if (communityData.category) {
    ...
    try {
      ...
      try {
        ...
        const response = await fetch(
          `/api/communities/members/${requestId}/approve`,
          {
            method: "POST",
            headers: {
        ...
        toast.error(
          `Failed to approve request: ${fetchError.message || "Network error"}`
        );
        return;
      }
      ...
      toast.error(
        `Failed to approve request: ${error.message || "Unknown error"}`
      );
  ...
  const handleReject = async (requestId: string) => {
    try {
      ...
      const response = await fetch(
        `/api/communities/members/${requestId}/approve?community_id=${community.id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleApproveAll = async () => {
    try {
      ...
      const supabase = getSupabaseBrowser();
      const pendingRequests = recentJoinRequests.filter(
        (r) => r.status === "pending"
      ); // String: 'pending'
      ...
      setRecentJoinRequests((prev) =>
        prev.filter((req) => !pendingRequests.some((pr) => pr.id === req.id))
      );
  ...
  const handleSaveDescription = async () => {
    ...
    try {
      ...
      setCommunity((prev) =>
        prev ? { ...prev, description: editedDescription.trim() } : null
      );
      setIsEditingDescription(false);
    ...
    try {
      ...
      if (data && data.description && typeof data.description === "string") {
        ...
        if (isEditingDescription) {
          ...
          setCommunity((prev) =>
            prev ? { ...prev, description: generatedDescription } : null
          );
        }
        toast.success("Description generated successfully!");
      } else {
        // If no description in response, use fallback
        const fallbackDescription = `${community.name} is a ${
          community.category
        } community focused on ${community.tags
          .slice(0, 3)
          .join(
            ", "
          )}. Join us to connect with like-minded individuals, share knowledge, and participate in activities related to our community interests.`;
        if (isEditingDescription) {
          setEditedDescription(fallbackDescription);
        } else {
          setCommunity((prev) =>
            prev ? { ...prev, description: fallbackDescription } : null
          );
        }
      ...
      const fallbackDescription = `${community?.name} is a ${
        ...
        community?.tags.slice(0, 3).join(", ") || "various topics"
      }. Join us to connect with like-minded individuals, share knowledge, and participate in activities related to our community interests.`;
      if (isEditingDescription) {
        setEditedDescription(fallbackDescription);
      } else {
      ...
      toast.warning(
        "Using fallback description. AI generation encountered an issue."
      );
    } finally {
  ...
  return (
    <PageTransition>
              ...
              <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
                <CardContent className="p-6">
                  <div className="flex flex-col items-center text-center space-y-4">
                    ...
                    {communityId && (
                      <Link
                        href={`/communities/${communityId}/admin/edit`}
                        className="w-full"
                      >
              ...
              <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg flex-1 flex flex-col">
                <CardHeader className="pb-2">
                  ...
                  <div className="mt-1 flex items-center justify-between text-sm text-gray-500">
                    <span>
                      {
                        recentJoinRequests.filter((r) => r.status === "pending")
                          .length
                      }{" "}
                      pending
                    </span>
                ...
                <CardContent className="pt-0 flex flex-col">
                                ...
                                <div className="flex items-center gap-3 mt-1">
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span className="text-xs font-medium text-green-600">
                                      {request.points_count || 0}{" "}
                                      {(request.points_count || 0) === 1
                                        ? "point"
                                        : "points"}
                                    </span>
                                  </div>
                                  <div className="flex items-center gap-1">
                                    ...
                                    >
                                      {request.report_count || 0}{" "}
                                      {(request.report_count || 0) === 1
                                        ? "report"
                                        : "reports"}
                                    </span>
                                  </div>
                                  ...
                                  request.reports.length > 0 && (
                                    <div className="mt-2">
                                      ...
                                      >
                                        ...
                                        <span>
                                          View {request.reports.length}{" "}
                                          {request.reports.length === 1
                                            ? "Report"
                                            : "Reports"}
                                        </span>
                                      </Button>
                  ...
                  {communityId && (
                    <div className="mt-3">
                      <Link
                        href={`/communities/${communityId}/admin/requests`}
                        className="block"
                      >
                        <Button
                          variant="ghost"
                  ...
                  {isEditingDescription ? (
                    ...
                    <>
                      ...
                      <div className="mt-4 flex items-center justify-start text-sm text-gray-500">
                        <div className="flex items-center space-x-4">
                          <span>
                            Character count: {community?.description.length}
                          </span>
                          <span>•</span>
                          <span>
                            Last updated:{" "}
                      ...
                      <Link href={`/communities/${communityId}/admin/events`}>
                        <div className="group rounded-xl border border-gray-200 bg-white p-5 hover:border-purple-300 hover:shadow-md transition-all cursor-pointer">
                          ...
                          <p className="text-sm text-gray-600 mb-3">
                            Manage and track your community events
                          </p>
      ...
      {/* Reports Dialog */}
      <Dialog
        open={!!selectedUserReports}
        onOpenChange={() => setSelectedUserReports(null)}
      >
        <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedUserReports?.reports.length}{" "}
              {selectedUserReports?.reports.length === 1 ? "report" : "reports"}{" "}
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <Badge
                        variant="outline"
                        className="bg-red-50 border-red-200 text-red-700 mb-2"
                      >
                        Report #{index + 1}
                      </Badge>
