...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-lg border bg-transparent px-3 text-sm font-medium ring-offset-background transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 group-[.destructive]:border-red-300/40 group-[.destructive]:hover:border-red-400/30 group-[.destructive]:hover:bg-red-100 group-[.destructive]:hover:text-red-900 group-[.destructive]:focus:ring-red-400",
      className
    )}
    {...props}
  />
...
const ToastIcon = ({ variant }: { variant?: string }) => {
  ...
  switch (variant) {
    ...
    case "destructive":
      return <AlertTriangle className={cn(iconClasses, "text-red-600")} />
    case "warning":
      return <AlertCircle className={cn(iconClasses, "text-yellow-600")} />
    case "info":
      return <Info className={cn(iconClasses, "text-blue-600")} />
    default:
      return <CheckCircle className={cn(iconClasses, "text-gray-600")} />
  }
}
