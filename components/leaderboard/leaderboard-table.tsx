...
import { Trophy, Medal, Award, TrendingUp, Users, Flame, Star, Crown, Zap, Eye, MessageCircle } from "lucide-react"
...
export function LeaderboardTable({ data, isLoading, sortBy }: LeaderboardTableProps) {
    ...
    return (
      <Card className="glass-effect border-0 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Top performers ranked by {sortBy.replace("-", " ")}</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="relative">
                  ...
                  {user.badge !== "none" && (
                    <div
                      className={`absolute -top-1 -right-1 w-6 h-6 rounded-full ${getBadgeColor(user.badge)} flex items-center justify-center`}
                    >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    ...
                    <span className={`flex items-center gap-1 ${getActivityColor(user.weeklyActivity)}`}>
                      <Zap className="h-3 w-3" />
                      {user.weeklyActivity}% active
                    </span>
                    <span className="flex items-center gap-1">
                      <TrendingUp className="h-3 w-3 text-green-500" />+{user.growth}%
                    </span>
                  </div>
                </div>
                ...
                <div className="hidden md:flex items-center gap-6 text-sm">
                  ...
                  <div className="text-center">
                    <div className="font-semibold">{user.communitiesJoined}</div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 space-y-4 animate-fade-in">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={getActivityColor(user.weeklyActivity)}>{user.weeklyActivity}%</span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div className="text-center p-3 bg-white/50 dark:bg-gray-800/50 rounded-lg">
                      <div className="font-semibold text-lg">{user.monthlyPosts}</div>
                      <div className="text-xs text-muted-foreground">Monthly Posts</div>
                    </div>
                    <div className="text-center p-3 bg-white/50 dark:bg-gray-800/50 rounded-lg">
                      <div className="font-semibold text-lg">{user.eventsAttended}</div>
                      <div className="text-xs text-muted-foreground">Events Attended</div>
                    </div>
                    <div className="text-center p-3 bg-white/50 dark:bg-gray-800/50 rounded-lg">
                      <div className="font-semibold text-lg">{user.communitiesJoined}</div>
                      <div className="text-xs text-muted-foreground">Communities</div>
                    </div>
                  </div>
                </div>
